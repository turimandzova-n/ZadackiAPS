...
public class Kumanovski {
	public static void main(String args[]) throws IOException{
		...
		for(int i=0; i<n; i++) {
			...
			String kumanovski=pom[0];
			String literaturno=pom[1];
			tabela.insert(kumanovski, literaturno);
		}
		String tekst = br.readLine();
		String []del = tekst.split(" ");
		for(int i =0; i<del.length; i++) {
			String word = del[i];
			boolean last=Character.isAlphabetic(word.charAt(word.length()-1));
			...
			int h = tabela.search(word);
			if(h==-1) {
				System.out.print(del[i]+" ");
			}
			else {
				String zbor = tabela.buckets[h].value;
				if(first) {
