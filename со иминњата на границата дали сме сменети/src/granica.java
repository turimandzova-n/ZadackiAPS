...
public class granica {
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < M; i++) {
			...
			imeto = pom1[0];
			promena = pom1[1];
			tabela2.insert(imeto, promena);
			}
		String vrednost=null;
		String prebaruvanje = br.readLine();
		int h = tabela1.search(prebaruvanje);
		if(h==-1) {
			System.out.println("Ne postoi takva licnost");
		}
		else {
			vrednost=tabela1.buckets[h].value;
			int H = tabela2.search(vrednost);
			if(H==-1) {
